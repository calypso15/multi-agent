...
- **`config.py`** — TOML loading, `AgentConfig`/`GeneralConfig`/`CommandConfig`/`CommandAgentConfig` dataclasses, validation. Agents are defined via `[agents.<name>]` blocks, commands via `[commands.<name>]` blocks, with optional per-agent command overrides via `[commands.<name>.agents.<agent>]`. `DEFAULT_REVIEW_COMMAND` provides a fallback review command. `get_display_name()` derives display names. `GeneralConfig.backend` selects the LLM backend (`"claude-cli"` by default).
...
- **`consensus.py`** — Orchestration: propose phase, review phase, iteration loop. Receives an `AgentBackend` instance and calls `backend.run_agent()` for all agent interactions. Edit validation, deduplication, and proposal merging also live here.
- **`arbitration.py`** — Stall detection, contested edit finding, arbitration execution, dissent collection. Called from the iteration loop in `consensus.py` at well-defined points.
- **`context.py`** — Builds user prompts (file contents, reference file listing, review round proposals). File resolution (`resolve_file_args`). Separate from system prompts.
...
- **`output.py`** — Rich terminal formatting. Call `init_agent_styles(config.agents)` after loading config to set up display names and colors. Two optional display modes: `--verbose` (operational telemetry: tokens, tools, turns) and `--detail` (full edit content each round). Both use module-level flags (`_VERBOSE`, `_DETAIL`) toggled by `set_verbose()`/`set_detail()`.
...
- **Cascading config** — Settings cascade four levels: global → agent → command → command-agent. `GeneralConfig` holds defaults, `AgentConfig` and `CommandConfig` hold optional overrides (`None` = inherit), and `CommandAgentConfig` provides per-agent overrides within a specific command. `resolve_run_config()` in `config.py` resolves all settings once at the boundary, producing a frozen `ResolvedRunConfig` that is threaded through `consensus.py`. Per-agent settings (`weight`, `propose_model`, `review_model`, `timeout_seconds`, `allowed_tools`, `file_patterns`, `reference_directories`, `max_reference_size_kb`, `propose_max_turns`, `review_max_turns`) are resolved per-agent. Per-run settings (`max_rounds`, `min_severity`, `min_blocking_severity`, `consensus_threshold`) are resolved command > general. No ad-hoc resolution in orchestration code.
- **Weighted consensus** — Each agent has a `weight` (default 1) that determines how much its approval counts toward `consensus_threshold`. Weights can be set at the agent level or overridden per-command via `CommandAgentConfig`. `count_blocking_approvals()` sums weights instead of counting agents.
...
- **Ask command** — `ask` reuses the edit-based consensus loop by writing the question to a temp file (`.multi_agent_ask_answer.md`) that agents edit. After consensus, the answer is displayed and saved in the structured run log. The temp file is cleaned up. `DEFAULT_ASK_COMMAND` in `config.py` provides the default prompt.
