...
- **Cascading config** — Settings cascade: global → agent → command. `GeneralConfig` holds defaults, `AgentConfig` and `CommandConfig` hold optional overrides (`None` = inherit). `resolve_run_config()` in `config.py` resolves all settings once at the boundary, producing a frozen `ResolvedRunConfig` that is threaded through `consensus.py`. Per-agent settings (`propose_model`, `review_model`, `timeout_seconds`, `allowed_tools`, `file_patterns`, `reference_directories`, `max_reference_size_kb`, `propose_max_turns`, `review_max_turns`) are resolved per-agent. Per-run settings (`max_rounds`, `min_severity`, `min_blocking_severity`, `consensus_threshold`) are resolved command > general. No ad-hoc resolution in orchestration code.
...
- **Ask command (two-file Q&A)** — `ask` reuses the entire edit-based consensus loop by writing the question to two files: `.multi_agent_ask_question.md` (preserved as-is) and `.multi_agent_ask_answer.md` (edited by agents). After consensus, the answer file contains the final merged answer. Both files persist until the next `ask` invocation so the result survives closing the console. `DEFAULT_ASK_COMMAND` in `config.py` provides the default prompt.
